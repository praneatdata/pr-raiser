...
def _seed(fake, month=None, total=None):
    fake.h["prlb:2026-08"] = dict(month or {})
...
def test_message_ranks_and_totals():
    ...
    with fake.patched():
        msg = leaderboard.build_message("prlb:2026-08", "August 2026")
    ...
    assert "*28* PRs raised in August" in msg and f"*{all_time}* all-time" in msg
...
def test_maintainer_is_counted_but_not_ranked():
    ...
    assert "*8* PRs raised in August" in msg             # and included in the totals
...
def test_singular_pr_wording():
    ...
    with fake.patched():
        msg = leaderboard.build_message("prlb:2026-08", "August 2026")
    assert "<@U1> — *1* PR\n" in msg + "\n" and "*1* PR raised in August" in msg


def test_no_activity_returns_no_message():
    fake = FakeKV()
    with fake.patched():
        assert leaderboard.build_message("prlb:2026-08", "August 2026") is None
...
def test_post_monthly_skips_a_quiet_month_and_stays_retryable():
    ...
    client = MagicMock()
    with fake.patched():
        r = leaderboard.post_monthly(client, now=_at(2026, 9, 1))
        assert r["status"] == "no_activity"
        client.chat_postMessage.assert_not_called()
        # the month wasn't consumed, so a later run can still announce it
        _seed(fake, {"U1": 2}, {"U1": 2})
        client.chat_postMessage.return_value = {"ts": "1.1", "channel": "C1"}
        assert leaderboard.post_monthly(client, now=_at(2026, 9, 1))["status"] == "posted"
